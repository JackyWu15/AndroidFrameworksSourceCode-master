...
//安卓系统EGL用于适配openGL环境的数据类型，即通过改变ANativeWindow的数据，来控制本地窗口
struct ANativeWindow
{
    ...
    const int   minSwapInterval;//最小的交换时间

    /* max swap interval supported by this updated */
    const int   maxSwapInterval;//最大的交换时间

    /* horizontal and vertical resolution in DPI */
    const float xdpi;//水平方向密度
    const float ydpi;//垂直方向密度
     ...
     */
    int     (*setSwapInterval)(struct ANativeWindow* window,//设置交换间隔时间
                int interval);
    ...
    int     (*dequeueBuffer_DEPRECATED)(struct ANativeWindow* window,
                struct ANativeWindowBuffer** buffer);//申请一个buffer
    ...
    int     (*queueBuffer_DEPRECATED)(struct ANativeWindow* window,
                struct ANativeWindowBuffer* buffer);//当一块buffer渲染完成，调用此接口来unlock和post buffer
    ...
    int     (*query)(const struct ANativeWindow* window,
                int what, int* value);//向本地窗口查询相关信息
