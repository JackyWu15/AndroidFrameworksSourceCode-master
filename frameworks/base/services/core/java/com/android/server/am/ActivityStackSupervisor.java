...
public final class ActivityStackSupervisor implements DisplayListener {
    ...
    private ActivityStack mHomeStack;//管理的是Launcher相关的Activity栈

    /** The stack currently receiving input or launching the next activity. */
    private ActivityStack mFocusedStack;//管理的是当前显示在前台Activity的Activity栈

    /** If this is the same as mFocusedStack then the activity on the top of the focused stack has
     * been resumed. If stacks are changing position this will hold the old stack until the new
     * stack becomes resumed after which it will be set to mFocusedStack. */
    private ActivityStack mLastFocusedStack;//管理的是上一次显示在前台Activity的Activity栈。
    ...
    void startSpecificActivityLocked(ActivityRecord r,
        ...
        //每个Activity都会记录当前的进程名和用户id，如果这个进程存在，就通知进程将这个Activity启动起来
    ...
    final int startActivityLocked(IApplicationThread caller,
        ...
        ProcessRecord callerApp = null;//ProcessRecord用来描述一个应用程序的进程，并保存在AMS内部
        if (caller != null) {
            callerApp = mService.getRecordForAppLocked(caller);//caller指向ApplicationThread，即让ProcessRecord指向所运行的应用程序
            if (callerApp != null) {
