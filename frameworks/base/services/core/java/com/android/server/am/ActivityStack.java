...
final class ActivityStack {
    ...
    final boolean startPausingLocked(boolean userLeaving, boolean uiSleeping, boolean resuming,
        ...
        //将mPausingActivity和mPausingActivity指向Launcher，并把mResumedActivity值为空，意味着后面的流程要重新开启新的Activity
        ...
        //prev.app记录了这个activity所在的进程，prev.app.thread是IBinder
        ...
        if (mPausingActivity != null) {
            ...
            if (dontWait) {
                ...
                //定时，如果在这个时间内上面还没有发消息给AMS，让其启动新Activity，那AMS就认为没有消息过来，
                // 不然AMS一直等着会出现应用程序无响应
    ...
    //暂停步骤完成，开始下一个activity
    private void completePauseLocked(boolean resumeNext) {
        ActivityRecord prev = mPausingActivity;//定义Launcher为上一个
        if (DEBUG_PAUSE) Slog.v(TAG, "Complete pause: " + prev);

        if (prev != null) {
            prev.state = ActivityState.PAUSED;//将Launcher置为Pause状态
            if (prev.finishing) {
                ...
                prev = null;//将其置空，表示终止的Activity已经进入Pause状态
            }
            mPausingActivity = null;
        }
        ...
        if (resumeNext) {
            ...
            if (!mService.isSleepingOrShuttingDown()) {//系统是不是睡眠或关闭状态
    ...
    //当前方法会执行pause再执行resume，第一次mResumedActivity！=null，pause完成后，mResumedActivity==null，就会执行startSpecificActivityLocked开始下一个Activity的启动
    final boolean resumeTopActivityInnerLocked(ActivityRecord prev, Bundle options) {
        ...
        //mResumedActivity：当前激活的Activity，mLastPausedActivity：上一次中止的Activity，mPausingActivity：正在被终止的Activity
    ...
    //调用startActivityLocked将MainActivity放到栈顶，调用resumeTopActivitiesLocked，执行上一个activity的pause()后，执行下一个activity的resume
    final void startActivityLocked(ActivityRecord r, boolean newTask,
        ...
        task.addActivityToTop(r);//将MainActivity放栈顶
        task.setFrontOfTask();
