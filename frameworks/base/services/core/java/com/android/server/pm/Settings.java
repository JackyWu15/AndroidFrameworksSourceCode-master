...
final class Settings {
    ...
    PackageSetting addPackageLPw(String name, String realName, File codePath, File resourcePath,
        ...
        //每个应用程序的安装信息都是用PackageSetting对象来存储的，并以包名为key，将对象保存在mPackages这个HashMap中
        PackageSetting p = mPackages.get(name);
        //存在这个包名的应用PackageSetting
        if (p != null) {
            //Linux用户id和appid相同，说明上一次分配过，直接返回PackageSetting
            if (p.appId == uid) {
                return p;
            }
        ...
        //不存在说明是第一次安装，新配置一个PackageSetting
    ...
    private boolean addUserIdLPw(int uid, Object obj, Object name) {
        //LAST_APPLICATION_UID是19999
        if (uid > Process.LAST_APPLICATION_UID) {
            return false;
        }
        //LAST_APPLICATION_UID是10000
        //说明用户的pid都在10000到19999之间，Android系统只分配9999个
    ...
    boolean readLPw(PackageManagerService service, List<UserInfo> users, int sdkVersion, boolean onlyCore) {
        ...
        if (mBackupSettingsFilename.exists()) {
            try {
                ...
                if (mSettingsFilename.exists()) {
                    // If both the backup and settings file exist, we ignore the settings since it might have been corrupted.
        ...
        //获取非独立用户id，即共享同一个用户id的应用程序
        final int N = mPendingPackages.size();
        for (int i = 0; i < N; i++) {
            final PendingPackage pp = mPendingPackages.get(i);
            Object idObj = getUserIdLPr(pp.sharedId);//获取共享用户安装信息
            //说明上一次所使用的Linux用户id是有效的，那么将这用户的信息进行保存
