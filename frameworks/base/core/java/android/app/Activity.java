...
public class Activity extends ContextThemeWrapper
    ...
    public void startActivityForResult(Intent intent, int requestCode, @Nullable Bundle options) {
        if (mParent == null) {
            //1，Instrumentation用来监控应用程序和系统之间的交互操作，即系统要启动应用程序，那么使用Instrumentation来启动
            //2，mMainThread为ActivityThread，表示了一个应用进程，即每启动一个应用程序，系统都会创建一个，这里假设是点击桌面的跳转，那么mMainThead即表示了Launcher这个应用程序
            //3，mMainThread.getApplicationThread()取得的是ApplicationThread，ActivityThread的内部类，是一个Bunder对象，
            //   先将ApplicationThread传给Instrumentation，Instrumentation会传递给AMS，用于ActivityThread和AMS之间的通讯。
            //4，每个启动了的Activity，在AMS中都有对应一个ActivityRecord来记录Activity的运行状况，这里的mToken为IBinder类型，同样传递给AMS，用来指向ActivityRecord
