...
public:
    sp<JavaBBinder> get(JNIEnv* env, jobject obj)
    {
        ...
        if (b == NULL) {
            b = new JavaBBinder(env, obj);//构造JavaBBinder
            mBinder = b;
...
static void android_os_Binder_init(JNIEnv* env, jobject obj)
{
    ...
    //将JavaBBinderHolder地址保存到mObject中
...
//AndroidRuntime启动虚拟机，注册Binder本地函数
static int int_register_android_os_Binder(JNIEnv* env)
{
    jclass clazz;

    clazz = env->FindClass(kBinderPathName);//找"android/os/Binder"类
    LOG_FATAL_IF(clazz == NULL, "Unable to find class android.os.Binder");

    //gBinderOffsets为bindernative_offsets_t结构体，将Binder类信息保存到结构体中
...
static int int_register_android_os_BinderProxy(JNIEnv* env)
{
    ...
    gBinderProxyOffsets.mConstructor= env->GetMethodID(clazz, "<init>", "()V");//保存构造Id
    ...
    gBinderProxyOffsets.mObject = env->GetFieldID(clazz, "mObject", "J");//保存类
