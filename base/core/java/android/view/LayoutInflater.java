...
public abstract class LayoutInflater {
     ...
     *        <em>attachToRoot</em> is true), or else simply an object that
     *        provides a set of LayoutParams values for root of the returned
     *        hierarchy (if <em>attachToRoot</em> is false.)父布局
     * @param attachToRoot Whether the inflated hierarchy should be attached to
     *        the root parameter? If false, root is only used to create the
     *        correct subclass of LayoutParams for the root view in the XML.
     * @return The root View of the inflated hierarchy. If root was supplied and
     *         attachToRoot is true, this is root; otherwise it is the root of
     *         the inflated XML file.//是否将解析好的视图加到父布局中
     */
        ...
        synchronized (mConstructorArgs) {
            ...
            try {
                ...
                if (TAG_MERGE.equals(name)) {
                    ...
                    //将解析temp下的所有子view，将子view添加到temp中
    ...
    View createViewFromTag(View parent, String name, AttributeSet attrs, boolean inheritContext) {
        ...
        try {
            ...
            if (view == null) {
                ...
                try {
                    //系统内置控件的解析，如TextView就是name
                    //这里实际上调用了PhoneLayoutInflater的onCreateView，拼接了前缀，再调用createView
                    if (-1 == name.indexOf('.')) {
                        view = onCreateView(parent, name, attrs);
                    } else {
                    //自定义控件的解析，如com.hechuangwu.widget.MyView就是name，不需要拼接前缀
